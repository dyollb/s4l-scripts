...
try:
    ...
    image_field_importer = analysis.extractors.ImageFieldImporter(inputs=[])
    ...
    conductivity_tensor_mapping_evaluator = (
        analysis.evaluators.ConductivityTensorMappingEvaluator(inputs=inputs)
    )
    ...
    inputs = [image_field_importer.Outputs["tensor_s4l.nii"]]
    fraction_anisotropy_evaluator = analysis.evaluators.FractionAnisotropyEvaluator(
        inputs=inputs
    )
    ...
    inputs = [image_field_importer.Outputs["tensor_s4l.nii"]]
    eigen_vector_evaluator = analysis.evaluators.EigenVectorEvaluator(inputs=inputs)
